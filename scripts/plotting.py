...
def plot_examples():
    ...
    ytitles = ["Sunspots", "CO2 (ppm)", "$ (CAD)", "Passengers"]
    fig = make_subplots(
        rows=2,
        cols=2,
...
def plot_stationary():
    """Plot examples of time series"""
    csvs = ["co2", "sunspots", "white-noise", "weight"]
    titles = [
        "Monthly mean atmospheric C02",
        "Annual sunspots",
        "White noise",
    ...
    ytitles = ["Sunspots", "CO2 (ppm)", "White noise", "Weight (kg)"]
    fig = make_subplots(
        rows=2,
        cols=2,
        subplot_titles=titles,
...
def plot_acf_pacf(type="ar", nsample=200, nlags=20):
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=(title, "ACF & PACF"))
    fig.add_trace(
        go.Scatter(
